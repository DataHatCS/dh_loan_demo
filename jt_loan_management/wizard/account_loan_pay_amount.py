...
class AccountLoanPayAmount(models.TransientModel):
    ...
    def invoice_line_vals(self, line):
        ...
        interest_prod_id = config_obj.get_param('jt_loan_management.interest_prod_id')
    ...
    def create_payment(self, invoice, date, amount, journal):
        if invoice:
            ...
            if payment:
                ...
                # payment.reconciled_invoice_ids = [(4, invoice.id)]
                #payment.has_invoices = True
            return payment
    ...
    def create_invoice_with_payment_date(self, loan_line, journal_id, account_id):
        ...
        invoice = self.env['account.move'].create({
            ...
            'name':str(loan_line.loan_id.name)+'-'+str(loan_line.loan_id.inv_counter),
    ...
    def create_invoice(self, line, date):
        ...
        loan_acc_rec_id = config_param_obj.get_param('jt_loan_management.acc_rec_id')
        ...
        loan_jou_id = config_param_obj.sudo().get_param('jt_loan_management.loan_jou_id')
        if not loan_jou_id:
    ...
    def run(self):
        ...
        loan_jou_id = ir_config_param_obj.get_param('jt_loan_management.loan_jou_id')
        if not loan_jou_id:
            raise Warning(_("Please Configure Loan Journal from Loans -> Configurations -> Settings!"))
        ...
        if self.extra_payment == 'pay_1' and not self.cancel_loan:
            ...
            # if extra_amount > 0 and covered_line:
            #     covered_line_seq = covered_line.sequence
            #     next_lines = loan_line_obj.search([('sequence', '>', covered_line_seq), ('loan_id', '=', loan.id)],
            #                                       order='sequence desc')
            #     for loan_line in next_lines:
            #         loan_line.sequence = loan_line.sequence + 1

            #     before_line = loan_line_obj.search([('loan_id', '=', loan.id),
            #                                     ('sequence', '=', covered_line_seq)], limit=1)
            #     line_data = {
            #         'loan_id': loan.id,
            #         'sequence': covered_line_seq + 1,
            #         'payment_amount': extra_amount,
            #         'principal_amount': extra_amount,
            #         'interests_amount': 0,
            #         'date': date,
            #         'pending_principal_amount': before_line.pending_principal_amount - \
            #                                     before_line.principal_amount
            #     }
            ...
            if all_lines_wihout_moves:
                ...
                if self.amount and date:
                    ...
                    if first_line:
                        ...
                        # for n_loan_line in next_lines:
                        #     n_loan_line.sequence = n_loan_line.sequence + 1
